...
# Load .txt files, which are supposed to be Fourier Transformed
imx = np.loadtxt('E_field_x100.txt')
imy = np.loadtxt('E_field_y100.txt')
imz = np.loadtxt('E_field_z100.txt')
...
def plotSignalAndSpectrum(F, xf, yf, signal, myFilename):
    ...
    im2 = axarr[1,].imshow(abs(F),
               ...
               norm = LogNorm(vmin = 0.998999, vmax = 0.99999999999999) #vmin = 0.996282276693, vmax = 0.999999999999999999
               )
...
if __name__ == '__main__':
    ...
    spectrumx, xfx, yfx = makeSpectrum(imx, x[1] - x[0], y[1] - y[0])
    spectrumy, xfy, yfy = makeSpectrum(imy, x[1] - x[0], y[1] - y[0])
    spectrumz, xfz, yfz = makeSpectrum(imz, x[1] - x[0], y[1] - y[0])


    fig, axarr = plt.subplots(2, 1) # create subplot array of 2 x 2
    plotSignalAndSpectrum(spectrumx, xfx, yfx, imx, "FFT_NoPrecalc_t=100_x")

    plt.clf()
    fig, axarr = plt.subplots(2, 1) # create subplot array of 2 x 2
    plotSignalAndSpectrum(spectrumy, xfy, yfy, imy, "FFT_NoPrecalc_t=100_y")

    plt.clf()
    fig, axarr = plt.subplots(2, 1) # create subplot array of 2 x 2
    plotSignalAndSpectrum(spectrumz, xfz, yfz, imz, "FFT_NoPrecalc_t=100_z")
